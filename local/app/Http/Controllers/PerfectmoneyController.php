...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
   ...
   public function avigher_perfectstatus(Request $request) 
	{
		$default = DB::table('avig_language')
	              ->where('lang_default','=',1)
		           ->get();
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
   ...
   if (!isset($data['PAYMENT_ID']) || !isset($data['PAYEE_ACCOUNT']) || !isset($data['PAYMENT_AMOUNT']) || !isset($data['PAYMENT_UNITS']) || !isset($data['PAYMENT_BATCH_NUM']) || !isset($data['PAYER_ACCOUNT']) || !isset($data['TIMESTAMPGMT'])) 
   {
     return view('cancel');
   }
	...
	public function avigher_perfectmoney(Request $request) 
	{
		$default = DB::table('avig_language')
	              ->where('lang_default','=',1)
		           ->get();
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
		...
		$paymentAccount = $data['PAYMENT_AMOUNT'];
		$paymentUnits = $data['PAYMENT_UNITS'];
		$paymentBatchNum = $data['PAYMENT_BATCH_NUM'];
		$payerAccount = $data['PAYER_ACCOUNT'];
		$timestampPGMT = $data['TIMESTAMPGMT'];
		$v2Hash = $data['V2_HASH'];
		
		$password_account = "48h2B42qcZkNzVTEBCwUxlCdu";
		...
		DB::update('update product_orders set payment_type="perfectmoney",license_start_date="'.$start_date.'",license_end_date="'.$end_date.'" where status="completed" and ord_id = ?', [$ord_id]);
		...
		$finder = 1;
		$datta = array('paymentBatchNum' => $paymentBatchNum, 'cid' => $cid, 'finder' => $finder);
		return view('perfectmoney_success')->with($datta);
