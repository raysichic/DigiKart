...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
		...
		$site_logo=$url.'/local/images/media/settings/'.$setting[0]->site_logo;
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
	...
	$razordata = [
			...
			"image"             => $url.'/local/images/media/settings/'.$setts[0]->site_logo,
			"prefill"           => [
			"name"              => $details_namer,
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
	...
	if(Auth::check()) {
	 ...
	 $data = array('cart_views_count' => $cart_views_count, 'cart_views' => $cart_views, 'setts' => $setts, 'admin_details' => $admin_details, 'countries_count' => $countries_count, 'cany_check_value' => $cany_check_value);
	...
	public function avigher_view_cart()
	{
	   ...
	   $cart_views = DB::table('product_orders')
	    ...
	    $data = array('cart_views_count' => $cart_views_count, 'cart_views' => $cart_views, 'setts' => $setts, 'admin_details' => $admin_details,  'cany_check_value' => $cany_check_value);
	...
	public function generatestring($length = 35) {
	...
	public function avigher_item_cart(Request $request)
	{
	   ...
	   $user_id = $data['user_id'];
	   $purchase_code = $this->generatestring();
	   $item_id = $data['item_id'];
	   $item_token = $data['item_token'];
	   $item_user_id = $data['item_user_id'];
	   ...
	   if(empty($check))
	   {
	    DB::insert('insert into product_orders (user_id,item_id,purchase_code,item_user_id,item_token,licence_type,price,total,status) values (?, ?, ?, ?, ?, ?,?,?,?)', [$user_id,$item_id,$purchase_code,$item_user_id,$item_token,$licence,$price,$price,$status]);
	   }
	   else
	   {
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
	...
	return view('add-item', ['category_count' => $category_count, 'framework_count' => $framework_count, 'language' => $language, 'cany_check_value' => $cany_check_value, 'browser' => $browser, 'graphics' => $graphics, 'adobe_cs' => $adobe_cs]);
	}
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }			
	...
	/* amazon s3 */
	   ...
	   $check_s3_meta = DB::table('settings_meta')
	...
	return view('edit-item', ['edit' => $edit, 'settings' => $settings, 'viewcount' => $viewcount, 'browser' => $browser, 'category_count' => $category_count, 'video_status' => $video_status, 'framework_count' => $framework_count, 'language' => $language, 'graphics' => $graphics, 'adobe_cs' => $adobe_cs, 'site_file_upload_by' => $site_file_upload_by]);
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
	...
	$check_item = DB::table('products')
		...
		$item_again = DB::select('select * from products_meta where item_token = ?',[$token]);
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }	
	   ...
	   $items_count = DB::table('products')
					...
					->count();
	   $settings = DB::select('select * from settings where id = ?',[1]);
	   return view('my-items', ['items_count' => $items_count, 'settings' => $settings, 'cany_check_value' => $cany_check_value]);
	...
	public function view_shopping()
	{
	   ...
	   $items_count = DB::table('product_orders')
					...
					->count();
	   $settings = DB::select('select * from settings where id = ?',[1]);
	   return view('my-shopping', ['items_count' => $items_count, 'settings' => $settings, 'setts' => $setts, 'url' => $url, 'cany_check_value' => $cany_check_value]);
	...
	public function view_orders()
	{
	   ...
	   $items_count = DB::table('product_orders')
					...
					->count();
	   $settings = DB::select('select * from settings where id = ?',[1]);
	   return view('my-orders', ['items_count' => $items_count, 'settings' => $settings, 'setts' => $setts, 'url' => $url, 'cany_check_value' => $cany_check_value]);
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
	   ...
	   $order_count = DB::table('product_orders')
		...
		if(!empty($limited_stock))
		{
			if($limited_stock > $downloaded_stock)
			{
			   ...
			   if($site_file_upload_by == "s3_server")
				...
				$myFile = Storage::disk('s3')->url($item_count[0]->main_file);
			   ...
			   $myFile = base_path('images/media/itemfile/').$item_count[0]->main_file;
    	       $headers = ['Content-Type: application/octet-stream'];
    	       $newName = uniqid().time().'.zip';
               return response()->download($myFile, $newName, $headers);
			   }
			   ...
			   //return redirect($url.'/local/images/media/itemfile/'.$item_count[0]->main_file);
		...
		else
		{
		   ...
		   if(!empty($item_count[0]->main_file))
		   {
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
	...
	$razordata = [
			...
			"name"              => $details_namer,
			"description"       => '',
			"image"             => $url.'/local/images/media/settings/'.$settings[0]->site_logo,
			"prefill"           => [
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
	...
	/*$category = DB::table('category')
	   ...
	   $check_sett_waterstatus = DB::table('settings_meta')
	   ...
	   $check_sett_water = DB::table('settings_meta')
	   ...
	   $image = Input::file('preview_image');
		...
		$savefname=$filename;
			...
			{
			$img = Image::make($url.'/local/images/media/preview/'.$filename);
		...
		$save_thumb=$fileename;
			if($site_watermark_status == "on")
			{
			$img = Image::make(base_path('images/media/thumbnail/'.$fileename));
				...
				foreach($files as $file){
					...
					else
					{
						...
						$file->move($destinationPath, $picture);
							if($site_watermark_status == "on")
							{
							$img = Image::make($url.'/local/images/media/screenshots/'.$picture);
							$img->insert($url.'/local/images/media/settings/'.$site_watermark,'center');
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
	   ...
	   $product_details = DB::table('products')->where('item_token','=',$token)->get();
		...
		$agin_file = $product_details[0]->downloaded + 1;
	   ...
	   $check_s3_meta = DB::table('settings_meta')
		...
		/* amazon s3  */
			 ...
			 if($product_details[0]->item_type == "yes")
			 {
			    DB::update('update products set downloaded="'.$agin_file.'" where item_token = ?', [$token]);
				
				
				if($site_file_upload_by == "s3_server")
				{
				   $myFile = Storage::disk('s3')->url($product_details[0]->main_file);
				   $newName = uniqid().time().'.zip';
				   /*$headers = ['Content-Type: application/octet-stream'];
    			   $newName = uniqid().time().'.zip';
				   return response()->download($myFile, $newName, $headers);*/
				...
				$myFile = base_path('images/media/itemfile/').$product_details[0]->main_file;
    			$headers = ['Content-Type: application/octet-stream'];
    			$newName = uniqid().time().'.zip';
    			return response()->download($myFile, $newName, $headers);
				}
	...
	/*$category = DB::table('category')
	  ...
	  /* graphic */
	   ...
	   $check_s3_meta = DB::table('settings_meta')
		...
		if($site_watermark_status == "on")
			{
			$img = Image::make($url.'/local/images/media/thumbnail/'.$fileename);
			 $img->insert($url.'/local/images/media/settings/'.$site_watermark,'center');
