    ...
    protected function settings(Request $request)
    {
		...
		if($data['purchase_code']!="")
		{
		   ...
		   $check_user_meta =  DB::table('settings_meta')
			...
			{
			   DB::update('update settings_meta set sett_meta_value="'.base64_encode($data['purchase_code']).'" where sett_meta_key="cc_token" and sett_meta_id = ?', [73]);
			}
			else
			{
			DB::insert('insert into settings_meta (sett_meta_id,sett_meta_key,sett_meta_value) values (?, ?, ?)', [73,"cc_token",base64_encode($data['purchase_code'])]);
		...
		if($data['buyer_name']!="")
		{
		   ...
		   $check_user_meta =  DB::table('settings_meta')
			...
			if(!empty($check_user_meta))
			{
			   DB::update('update settings_meta set sett_meta_value="'.base64_encode($data['buyer_name']).'" where sett_meta_key="cc_byname" and sett_meta_id = ?', [74]);
			}
			else
			{
			DB::insert('insert into settings_meta (sett_meta_id,sett_meta_key,sett_meta_value) values (?, ?, ?)', [74,"cc_byname",base64_encode($data['buyer_name'])]);
