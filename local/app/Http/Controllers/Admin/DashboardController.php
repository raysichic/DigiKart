...
class DashboardController extends AdminController {
	...
	public function index()
	{
       ...
       $last_date6=date("Y-m-d", strtotime("-6 days"));
                      ...
                      $date1 = DB::table('product_checkout')
			           ->where('payment_date','=', $curr_date)
					   ->count();
					   $date2 = DB::table('product_checkout')
			           ->where('payment_date','=', $last_date1)
					   ->count();
					   $date3 = DB::table('product_checkout')
			           ->where('payment_date','=', $last_date2)
					   ->count();
					   $date4 = DB::table('product_checkout')
			           ->where('payment_date','=', $last_date3)
					   ->count();
					   $date5 = DB::table('product_checkout')
			           ->where('payment_date','=', $last_date4)
					   ->count();
					   $date6 = DB::table('product_checkout')
			           ->where('payment_date','=', $last_date5)
					   ->count();
					   $date7 = DB::table('product_checkout')
