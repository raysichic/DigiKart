    ...
    protected function editsettings(Request $request)
    {
		...
		if($data['site_file_upload_by']!="")
		{
		   ...
		   $check_user_meta =  DB::table('settings_meta')
			...
			{
			   DB::update('update settings_meta set sett_meta_value="'.$data['site_file_upload_by'].'" where sett_meta_key="site_file_upload_by" and sett_meta_id = ?', [95]);
			}
			else
			{
			DB::insert('insert into settings_meta (sett_meta_id,sett_meta_key,sett_meta_value) values (?, ?, ?)', [95,"site_file_upload_by",$data['site_file_upload_by']]);
