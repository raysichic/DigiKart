...
class SettingsController extends Controller
{
   ...
   /* new */
    ...
    protected function editsettings(Request $request)
    {
		...
		$waterimages = Input::file('site_watermark');
        if($waterimages!="")
		{	
            $settingbanphotos="/media/settings/";
			...
			$destinationbanPaths=base_path('images'.$settingbanphotos);
                ...
                /*Image::make($banimages->getRealPath())->resize(1920, 500)->save($banpaths);*/
				Input::file('site_watermark')->move($destinationbanPaths, $banfilenames);
		...
		if(!empty($data['site_watermark']))
		{
		   ...
		   $check_user_meta =  DB::table('settings_meta')
			...
			if(!empty($check_user_meta))
			{
			   DB::update('update settings_meta set sett_meta_value="'.$save_water.'" where sett_meta_key="site_watermark" and sett_meta_id = ?', [77]);
			}
			else
			{
			DB::insert('insert into settings_meta (sett_meta_id,sett_meta_key,sett_meta_value) values (?, ?, ?)', [77,"site_watermark",$save_water]);
		...
		if(!empty($data['site_watermark_status']))
		{
		   ...
		   $check_user_meta =  DB::table('settings_meta')
			...
			if(!empty($check_user_meta))
			{
			   DB::update('update settings_meta set sett_meta_value="'.$data['site_watermark_status'].'" where sett_meta_key="site_watermark_status" and sett_meta_id = ?', [78]);
			}
			else
			{
			DB::insert('insert into settings_meta (sett_meta_id,sett_meta_key,sett_meta_value) values (?, ?, ?)', [78,"site_watermark_status",$data['site_watermark_status']]);
		...
		if(!empty($data['site_verify_purchase']))
		{
		   ...
		   $check_user_meta =  DB::table('settings_meta')
			...
			if(!empty($check_user_meta))
			{
			   DB::update('update settings_meta set sett_meta_value="'.$data['site_verify_purchase'].'" where sett_meta_key="site_verify_purchase" and sett_meta_id = ?', [100]);
			}
			else
			{
			DB::insert('insert into settings_meta (sett_meta_id,sett_meta_key,sett_meta_value) values (?, ?, ?)', [100,"site_verify_purchase",$data['site_verify_purchase']]);
