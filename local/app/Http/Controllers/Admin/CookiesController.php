...
class CookiesController extends Controller
{
	...
	public function showform() {
	 ...
	 $check_sett_cookie = DB::table('settings_meta')
		...
		$check_sett_cookie_position = DB::table('settings_meta')
    ...
    protected function editsettings(Request $request)
    {
		...
		if($data['site_cookie']!="")
		{
		   ...
		   $check_user_meta =  DB::table('settings_meta')
			...
			{
			   DB::update('update settings_meta set sett_meta_value="'.$data['site_cookie'].'" where sett_meta_key="site_cookie" and sett_meta_id = ?', [75]);
			}
			else
			{
			DB::insert('insert into settings_meta (sett_meta_id,sett_meta_key,sett_meta_value) values (?, ?, ?)', [75,"site_cookie",htmlentities($data['site_cookie'])]);
		...
		if($data['site_cookie_position']!="")
		{
		   ...
		   $check_user_meta =  DB::table('settings_meta')
			...
			if(!empty($check_user_meta))
			{
			   DB::update('update settings_meta set sett_meta_value="'.$data['site_cookie_position'].'" where sett_meta_key="site_cookie_position" and sett_meta_id = ?', [76]);
			}
			else
			{
			DB::insert('insert into settings_meta (sett_meta_id,sett_meta_key,sett_meta_value) values (?, ?, ?)', [76,"site_cookie_position",htmlentities($data['site_cookie_position'])]);
